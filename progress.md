...
- Product data now lives in Firebase (Firestore + Storage) instead of a static file — fetched at runtime via `js/modules/productsStore.js`
...
- Product Admin (`admin.html`, developer-only, not linked from the site) — a real, password-protected dashboard (Firebase Authentication): sign in, then add/edit/delete products and upload/remove photos, all saved immediately and live on the public site on next reload. `js/data/seed-products.js` powers its one-click "Import Starter Catalog" button (28 placeholder products) if you want a starting point instead of adding from scratch.
...
- [ ] **Complete `FIREBASE_SETUP.md`** (one-time, ~15 min) — creates the Firebase project, your one admin login, and the security rules that make `admin.html` actually secure. Nothing in Firestore/Storage/admin.html works until this is done.
...
- `admin.html` has no public sign-up — only the one account you create during `FIREBASE_SETUP.md` can sign in or change anything, enforced by Firestore/Storage security rules server-side (not just by the page being unlinked).
